...
### 🎨 프론트엔드 (혜성님) - [COMPLETED]
- [x] **인프라 총괄**: Azure App Service 및 Static Web Apps 환경 구축 완료.
- [x] **SDK 통합**: 카카오 지도 + 카카오 로그인(실제 리다이렉트 방식) 연동 완료.
- [x] **핵심 UI**: 틴더형 스와이프 UI, 투표 목록, 생성(장소 검색), 결과 페이지 구현 완료.

### ⚙️ 백엔드 A (인증 & 세션) - [COMPLETED]
- **카카오 OAuth 서버**: 인가 코드 기반 토큰 교환 및 서비스 JWT 발급 로직 구축 완료.
- **GitHub 배포**: 팀장님이 세팅한 Azure 환경에 코드 Push로 즉시 배포.
